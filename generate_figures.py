...
# ---- Figure 1 : Trajectoire temporelle du modèle logistique ----
def logistique(x, r):
    return r * x * (1 - x)
...
plt.ylabel("$x_n$")
plt.title(f"Modèle logistique, $r = {r}$, $x_0 = {x0}$")
plt.grid(alpha=0.3)
plt.tight_layout()
...
plt.xlabel("Paramètre $r$")
plt.ylabel("Valeurs asymptotiques de $x$")
plt.title("Diagramme de bifurcation du modèle logistique")
plt.xlim(2.8, 4.0)
plt.grid(alpha=0.2)
plt.tight_layout()
...
# ---- Figure 4 : Exposant de Lyapunov pour le modèle logistique ----
...
lyap_vals = []
for r in r_vals:
    # on prend une moyenne sur quelques conditions initiales pour lisser
    lyap_mean = 0
...
plt.title("Exposant de Lyapunov du modèle logistique")
plt.xlim(2.8, 4.0)
plt.grid(alpha=0.3)
plt.tight_layout()
...
print("✅ Figures générées : figure_trajectoire.png, figure_bifurcation.png, figure_lorenz.png, figure_lyapunov.png")